...
/**
 * P8-O02: Oracle運用検証（Phase 8指示 §12.5）。
 * - 必須suite（P1-O01〜P8-O01の8件）が各1件存在すること
 * - 証跡書込みが現行Phase（P8）のみで、書込み先がartifacts/phase-8/oracle-result.mdだけであること
 * - 実行前後でartifacts/phase-1〜phase-7のSHA-256が不変であること（実測はrun-oracle.mjsが行う）
 *
 * JDKとの実照合はnpm run test:oracleが行い、こちらはsuite構成・書込み範囲の契約を機械検証する。
 * Phase 5〜7時点の同契約はp5 / p6 / p7-review.test.tsがfixture構成を渡して検証し続ける。
 */

const ALL_PASSED = REQUIRED_SUITE_IDS.map((id: string) => ({ id, passed: true }))

describe('P8-O02 Oracle運用検証（suite構成と書込み範囲）', () => {
  it('P8-O02: 必須8 suite（P1-O01〜P8-O01）が各1件存在し、証跡書込みは現行PhaseのP8のみである', () => {
    ...
    // 過去Phase suite（P1〜P7）は照合のみ。P7-O01の証跡書込みはPhase 8着手に伴い停止した
    for (const id of ['P1-O01', 'P2-O01', 'P3-O01', 'P4-O01', 'P5-O01', 'P6-O01', 'P7-O01']) {
      const suite = SUITES.find((s: { id: string }) => s.id === id)!
      expect(suite.writeReportPath, id).toBeNull()
    }
    // 書込みは現行Phase（P8-O01）ただ1件、書込み先はartifacts/phase-8/oracle-result.mdだけ
    ...
    expect(writers[0]?.writeReportPath).toEqual(['artifacts', 'phase-8', 'oracle-result.md'])
    expect(writers[0]?.writeReportPath?.join('/')).toBe(CURRENT_PHASE_REPORT_PATH)
    // 過去Phase証跡へは書き込まない
    for (const suite of writers) {
      expect(suite.writeReportPath?.join('/')).not.toMatch(/phase-[1234567]/)
    }
    // P8 suiteの実体
    const p8 = SUITES.find((s: { id: string }) => s.id === 'P8-O01')!
    expect(p8.javaFile).toBe('OracleP8.java')
    expect(p8.expectedFile).toBe('expected-p8-from-core.json')
    // P4-O02のLong境界値照合はP4 suiteへ適用し続ける（ID再定義はしない）
    expect(BOUNDARY_SUITE_ID).toBe('P4-O01')
  })

  it('P8-O02: 過去artifacts不変検証の対象へartifacts/phase-7が含まれ、対象ディレクトリが実在する', () => {
    ...
    expect(PAST_ARTIFACT_DIRS).not.toContain('artifacts/phase-8')
  ...
  it('P8-O02: suite構成の欠落・重複・書込み先異常をFAILと判定する', () => {
    ...
    // 過去Phaseへ書き込む構成 → FAIL（P7-O01の書込み再開を検知する）
    const p7Writes = evaluate(
      SUITES.map((s: { id: string }) =>
        s.id === 'P7-O01'
          ? { ...s, writeReportPath: ['artifacts', 'phase-7', 'oracle-result.md'] }
          : s,
      ),
    )
    expect(p7Writes.configOnlyCurrentPhaseWrites).toBe(false)
    expect(p7Writes.o02Passed).toBe(false)
    ...
    // P8-O01の照合失敗 → FAIL
    const p8Failed = evaluateCurrentPhaseOracleIds({
      suiteResults: ALL_PASSED.map((r: { id: string; passed: boolean }) =>
        r.id === 'P8-O01' ? { ...r, passed: false } : r,
      ),
      pastArtifactsUnchanged: true,
    })
    expect(p8Failed.o01Passed).toBe(false)
    expect(p8Failed.overallPassed).toBe(false)
    // 1件でも比較失敗があればコマンド全体が失敗する
    expect(allSuitesPassed(ALL_PASSED)).toBe(true)
    expect(allSuitesPassed([...ALL_PASSED.slice(0, 7), { id: 'P8-O01', passed: false }])).toBe(false)
    expect(allSuitesPassed([])).toBe(false)
  })

  it('P8-O02: 結果欄が実判定から生成され、レポートへ組み込まれる', () => {
    const regression = ['P7-O01: PASS（照合のみ・証跡書込みなし）']
    ...
    expect(passSection).toContain('- P8-O01: PASS')
    expect(passSection).toContain('- P8-O02: PASS')
    expect(passSection).toContain(
      '必須8 suite（P1-O01 / P2-O01 / P3-O01 / P4-O01 / P5-O01 / P6-O01 / P7-O01 / P8-O01）が各1件存在（欠落・重複なし）: PASS',
    )
    expect(passSection).toContain('証跡書込みは現行Phase（P8）のみ')
    expect(passSection).toContain('artifacts/phase-8/oracle-result.md')
    expect(passSection).toContain('実行前後でartifacts/phase-1〜phase-7のSHA-256が不変: PASS')
    expect(passSection).toContain('総合判定: PASS')
    expect(passSection).toContain('P7-O01: PASS（照合のみ・証跡書込みなし）')
    // §12.5の照合方式注記（P8_MATCH_NOTES）が出力される
    expect(passSection).toContain('TreeMap（4引数版）だけは実entry順')
    expect(passSection).toContain('例外**型のみ**を契約として照合する')
    ...
    expect(failSection).toContain('- P8-O02: FAIL')
    expect(failSection).toContain('実行前後でartifacts/phase-1〜phase-7のSHA-256が不変: FAIL')
