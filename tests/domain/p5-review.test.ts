 ...
 * - 証跡書込みが現行Phase（P5）のみで、書込み先がartifacts/phase-5/oracle-result.mdだけであること
 * - 実行前後でartifacts/phase-1〜phase-4のSHA-256が不変であること（実測はrun-oracle.mjsが行う）
 *
...
describe('P5-O02 Oracle運用検証（suite構成と書込み範囲）', () => {
  it('P5-O02: 必須5 suite（P1-O01〜P5-O01）が各1件存在し、証跡書込みは現行PhaseのP5のみである', () => {
    ...
    expect(REQUIRED_SUITE_IDS).toEqual(['P1-O01', 'P2-O01', 'P3-O01', 'P4-O01', 'P5-O01'])
    ...
    // P5 suiteの実体
    const p5 = SUITES.find((s: { id: string }) => s.id === 'P5-O01')!
  ...
  it('P5-O02: 過去artifacts不変検証の対象へartifacts/phase-4が含まれ、対象ディレクトリが実在する', () => {
    ...
    expect(PAST_ARTIFACT_DIRS).not.toContain('artifacts/phase-5')
