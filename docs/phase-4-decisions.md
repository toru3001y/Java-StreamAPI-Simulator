...
## 10. Phase 4第2回再レビュー対応（2026-08-08追記）

第2回再レビュー指摘3件（証跡保全・完了報告の整合性）への判断。§1〜§9の記載は
削除・書換えせず保持する。対応範囲は指摘3件のみで、Phase 5の先行実装・
既存72 IDの変更・当初4件の修正ロジックの変更は行っていない。

### 10.1 E2Eキャプチャ証跡の書込み対象を現行Phaseへ限定（指摘1、§9.6の解消）

- `npm run test:e2e`のキャプチャtestが`artifacts/phase-1〜3`へ直接PNGを書き込み、
  実行後にHEAD内容を書き戻して復元する運用は、過去証跡の不変性をE2Eランナー自身が
  保証しておらず、未コミットのユーザー変更が書き戻しで失われる危険があった。
- 共通helper `e2e/capture-helper.ts` を新設し、証跡書込みを許可するPhaseを
  定数`CAPTURE_TARGET_PHASE`（現在は4）で1か所に定義した。
  Phase 5着手時はこの1か所だけを更新する。
- **P1〜P3のキャプチャtestは削除・skipせず、既存の画面操作をすべて実行する。**
  書込み対象外のPhaseでは`page.screenshot()`のbuffer取得だけを行い、
  追跡対象ファイルへ一切書き込まない。P4のキャプチャだけが
  `artifacts/phase-4/`へ書き込まれる。
- 各specへの変更は「`page.screenshot({ path })`直書き → helper呼出し」への
  最小置換のみで、キャプチャ以外のassertion・操作順・待機条件・test ID・
  視覚回帰画像・threshold・Playwright project構成は変更していない。
- これにより「一度書き換えてからHEAD内容を書き戻す」運用は**廃止**した。
  E2E実行前後の`artifacts/phase-1〜3`のSHA-256完全一致を、復元操作なしで
  実測確認する（結果は完了報告§17）。
- package.json / playwright.config.tsの変更は不要だった（helperのみで実現できたため）。

### 10.2 Oracle証跡へP4-O01〜O03の結果を明示（指摘2）

- 従来の`artifacts/phase-4/oracle-result.md`はP4-O01のPASSだけを明示し、
  P4-O02 / O03は関連テストファイル名の記載にとどまっていたため、
  証跡単体で3 IDの最終結果を確認できなかった。
- `oracle-lib.mjs`へ判定関数`evaluateOracleIds`と結果セクション生成
  `buildOracleIdSection`を追加し、`npm run test:oracle`の再実行でも同じ結果欄が
  生成されるようにした。**結果はすべて実測から導出し、固定文字列のPASSは出力しない**:
  - P4-O01: P4 suiteの照合結果（JDK 25実測値とCore期待値のJSON完全一致）から導出。
  - P4-O02: `verifyLongBoundaryStrings`の実結果（期待値・実測値の双方）から導出。
    正確な10進値（`9223372036854775807` / `-9223372036854775808`）と比較方式
    （10進文字列のまま完全一致比較、numberへ変換しない）を明示する。
  - P4-O03: suite構成（P1〜P3が`writeReportPath: null`で照合のみ）と、
    ランナー自身が全suite実行前後に実測する`artifacts/phase-1〜3`のSHA-256
    比較結果から導出する。
  - 総合判定: P4-O01〜O03のいずれかがFAILなら総合もFAILとし、
    コマンド全体を失敗させる。
- findAnyの観測要素とpeek + countの呼出し回数は、引き続き厳密比較の対象外
  （観測記録のみ）を維持する。P1〜P3のoracle-result.mdは生成しない。
- 新しいOracle IDは追加せず、既存P4-O02 / O03のテスト
  （`tests/domain/p4-review.test.ts`）を拡張して、生成レポートに各IDの結果・
  10進値・比較方式・総合判定が含まれること、FAILが正しく伝播することを検証した。

### 10.3 §7の視覚回帰差分記載への訂正

- §7の「差分がヘッダー副題と操作選択行に限られ」という記載は不正確だった。
  **P3のpeek画像（`p3-e10-peek-action.png`）には、副題・操作選択行に加えて
  Side Effectビューの説明文の一般化**（forEach対応に伴う見出しの対象操作表示と、
  説明文「peekのConsumerによる…」→「ConsumerによるSystem.out出力の履歴です」への変更）
  **も差分に含まれていた**（第1回再レビューで指摘され、完了報告§11は訂正済み。
  本節で判断記録側にも訂正を記録する。§7の本文自体は書換えず保持する）。
- 上記以外の領域（Pipeline・シミュレーション本体・コード・再生バー）に予期しない
  差分がないこと、thresholdを緩和していないことは従来の記載どおり。

### 10.4 完了報告の整合性修正（指摘3）

- 完了報告§4の見出しを72 ID（当初68 + 再レビュー追加4）が分かる表記へ変更し、
  68 IDと72 IDの使い分けを明文化した。
- `tests/domain/p4-*.test.ts`のファイル数を実数7へ訂正（旧記載は6）。
- P4-O01〜O03の結果、E2E恒久修正、push状態の時系列
  （第2回独立レビュー時点で`origin/phase-4` = `7a77a79`、`2c6a864` / `7a77a79`は
  現在origin/phase-4に存在、第2回是正作業ではpushしない）を記載した。
- 作業ツリー状態は「追跡対象はclean」「既知の未追跡ファイルあり」を区別して記載する。
- commit SHAは作成時点で確定済みのものだけをファイルへ記載し、
  第2回是正commit自身のSHAはreport-only commitと最終回答で報告する。

## 11. Phase 4第3回再レビュー対応（2026-08-08追記）

第3回再レビュー指摘2件への判断。§1〜§10は削除・書換えせず保持する。
E2E capture helper・各E2E spec・当初4件の修正ロジック・既存72 IDの意味・
Phase 5以降の実装には手を入れていない。

### 11.1 P4-O03のsuite構成判定の厳密化（指摘1）

- 従来の`configOnlyP4Writes`は「存在するsuiteのうちP4以外がwriteReportPath: null」
  だけを確認しており、P1-O01〜P3-O01がsuite構成から欠落してもP4-O03と総合判定が
  PASSになり得た（欠落は照合対象の縮小であり、PASSにしてはならない）。
- `evaluateOracleIds`へ必須suite判定を追加した。判定は3つに分離し、
  それぞれを実結果から導出する（固定文字列のPASSは出力しない）:
  1. **必須4 suiteが各1件存在**: `REQUIRED_SUITE_IDS`（P1-O01 / P2-O01 / P3-O01 /
     P4-O01）のそれぞれがsuite構成に**ちょうど1件**存在すること。欠落も重複もFAIL。
  2. **書込みはP4のみ**: writerがP4-O01ただ1件で、書込み先が
     `artifacts/phase-4/oracle-result.md`であり、P4以外の全suiteが
     writeReportPath: null（照合のみ）であること。書込み先の検証を追加した。
  3. **過去Phase証跡のSHA-256不変**: ランナーが実行前後に実測（従来どおり維持）。
- P4-O03は3判定すべての成立で初めてPASSとなり、いずれかの不成立で
  P4-O03 → 総合判定 → `npm run test:oracle`の順にFAILが伝播する。
- oracle-result.mdの結果欄では判定1と判定2を**別々の行**として表示する。
- P4-O03テストへ構成異常系を追加した: P1-O01 / P2-O01 / P3-O01 / P4-O01の各欠落、
  必須suiteの重複、P1〜P3への書込み先設定、P4の書込み先異常のそれぞれがFAILになること、
  正常な実SUITES構成がPASSになること、結果欄に2判定が分離して出力されること。
- 新しいOracle IDは追加していない（既存P4-O03の強化）。

### 11.2 完了報告の件数訂正（指摘2）

- 再レビュー追加分のit件数を最終実数へ更新した:
  P4-D41: 2件 / P4-D42: 2件 / P4-O02: 5件 / P4-O03: 4件（今回の構成異常系1件を含む）。
- P4-O02 / O03の検証内容欄へ、結果欄生成・FAIL伝播・レポート統合・suite欠落検出を明記した。
- push状態は時系列で区別して記載した: 第2回是正作業中はClaude Codeがpushしておらず、
  その後ユーザーのpush指示で`693e8b2` / `d8ab9ae`がpushされ、第3回独立レビュー時点の
  `origin/phase-4`は`d8ab9ae`だった。誰がpushしたかの推測は行っていない。
